...
type ConstellationContainer =
  ...
  member this.DeleteWithOptions (itemOptions: ItemRequestOptions option) (cancelToken: CancellationToken option) item =
    let options = itemOptions |> Option.toObj
    let token = cancelToken |> getCancelToken

    let id = AttributeHelpers.getIdFromTypeFrom item
